 ...
 */
	public interface List<T> : Collection<T> {
		...
		Q Reduce<T, Q>(Q a, IEnumerable<T> l, Func<Q, T, Q> f);
		IEnumerable<Q> Map<T, Q>(IEnumerable<T> l, Func<T, Q> f);
		T Find<T>(IEnumerable<T> r, Predicate<T> p);
		List<T> Filter<T>(IEnumerable<T> l, Predicate<T> p);
		IEnumerable<T> Divide<T>(IEnumerable<T> list, Predicate<T> p);
	}
	}
